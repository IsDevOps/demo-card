...
class LeadContactExport implements FromCollection,WithHeadings, WithEvents
{
    ...
    */
    ...
    public function collection()
    {
		...
		$leads_contacts = LeadContact::where('created_by',\Auth::user()->creatorId())->get();
                foreach ($leads_contacts  as $k => $contact) {
					
					unset($contact->created_by,$contact->id,$contact->campaign_id,$contact->business_id,$contact->updated_at,$contact->status, $contact->note );
